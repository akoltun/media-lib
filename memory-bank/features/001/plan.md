...
- `oxlint-tsgolint` необходим для работы type-aware линтера (`options.typeAware: true` и type-aware правила в `rules`)

**Проверка:** команда завершается с кодом `0`, `oxlint`, `oxfmt` и `oxlint-tsgolint` появляются в `devDependencies` в `package.json`.
...
| #   | Проверка                                                  | Команда                 |
| --- | --------------------------------------------------------- | ----------------------- |
| 0   | `oxlint`, `oxfmt` и `oxlint-tsgolint` в `devDependencies` | просмотр `package.json` |
...
| Файл               | Действие                                                                                                                 |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `package.json`     | Добавить скрипт `lint`; `devDependencies` обновляются через `bun add` (три пакета: `oxlint`, `oxfmt`, `oxlint-tsgolint`) |
